...
var Tixia = function(){
		...
		if(jQuery('select.nice-select').length > 0){
			...
			elementArry.forEach((item, index) => {
				...
				jQuery(item).parents().map((data,_) => {
					if(jQuery(jQuery(item).parents()[data]).css('position') === 'relative'){
						tempArry.push(jQuery(item).parents()[data]);
					}
				})
				if(jQuery(tempArry[0]).height() < (20 + jQuery(item).next().find('ul.list').height() + jQuery(item).next().height() + jQuery(item).next().position().top)){
	...
	var handleDatepicker = function(){
		let datePicker;
		$(() => {
		datePicker = $(".datePicker6").pDatePicker({
			// options here
		});
		});
