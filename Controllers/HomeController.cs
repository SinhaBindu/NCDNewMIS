...
namespace NCDNewMIS.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult FollowSuspSummaryNew2()
        {
            return View();
        }
        public ActionResult GetFollowupSummmaryNew2(string BlockId, string StartDate, string EndDate, string TypeOfPatient, string GenderId, string Ageyrs)
        {
            DataSet ds = new DataSet();
            DataTable tbllist = new DataTable();
            try
            {
                ds = SP_Model.Sp_FollowupSuspectedSummaryNew2(BlockId, StartDate, EndDate, TypeOfPatient, GenderId, Ageyrs);
                bool IsCheck = false;
                if (ds.Tables.Count > 0)
                {
                    IsCheck = true;
